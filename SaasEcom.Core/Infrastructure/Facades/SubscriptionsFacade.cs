...
namespace SaasEcom.Core.Infrastructure.Facades
{
    ...
    public class SubscriptionsFacade
    {
        ...
        /// Subscription
        /// </returns>
        public async Task<Subscription> SubscribeUserAsync(SaasEcomUser user, string planId, decimal taxPercent = 0, CreditCard creditCard = null)
        {
            ...
            if (string.IsNullOrEmpty(user.StripeCustomerId))
            {
                ...
                var stripeUser = (Customer) await _customerProvider.CreateCustomerAsync(user, planId, subscription.TrialEnd, cardToken);
        ...
        public async Task SubscribeUserNaturalMonthAsync(SaasEcomUser user, string planId, CreditCard card, decimal taxPercent = 0)
        {
            if (string.IsNullOrEmpty(user.StripeCustomerId))
            {
                ...
                var stripeUser = (Customer)await _customerProvider.CreateCustomerAsync(user, cardToken: card.StripeToken);
            ...
            else if (card != null && !string.IsNullOrEmpty(card.StripeToken))
            {
                ...
                var customer = (Customer)_customerProvider.UpdateCustomer(user, card);
                card.SaasEcomUserId = user.Id;
                card.StripeId = customer.DefaultSourceId;
                await _cardDataService.AddOrUpdateDefaultCardAsync(user.Id, card);
            }

            var stripeSubscription = (Subscription)_subscriptionProvider.SubscribeUserNaturalMonth(user, planId, GetStartNextMonth(), taxPercent);
            await _subscriptionDataService.SubscribeUserAsync(user, planId, (int?)null, taxPercent, stripeSubscription.Id.ToString());
        }
        ...
        private string GetStripeSubscriptionIdForNewCustomer(Customer stripeUser)
        {
            return stripeUser.Subscriptions.Count() > 0 ? 
